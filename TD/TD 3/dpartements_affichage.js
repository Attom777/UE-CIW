...
function go() {
  ...
  actions.initAndGo({ departments: departements, displayMode, filterString, includesRegions, filterMethod: 'for' });
...
state = {
  ...
  filterDepartments_for: function(depArray) {
    ...
    //console.log(this.currentFilterString);
    //console.log(model.includesRegions);

    // TODO: filtrer les éléments en ne gardant que ceux 
    //qui contiennent la chaîne de caractère depFilter

    function for_filter(x, a) {

      let len = model.departments.length;
      let isInDep = false;
      let isInReg = false;
      let isCheckCase = model.includesRegions;

      for (let i = 0; i < len; i++) {

        let isInDep = x[i][0].toLowerCase().includes(model.filterString.toLowerCase());
        let isInReg = x[i][2].toLowerCase().includes(model.filterString.toLowerCase());
  ...
  filterDepartments_filter: function(depArray) {
    ...
    function wordTest(word) {
      ...
      let depWordLower = word[0].toLowerCase();
      let regWordLower = word[2].toLowerCase();

      if (model.includesRegions) {
        return depWordLower.includes(model.filterString.toLowerCase()) || regWordLower.includes(model.filterString.toLowerCase());
      } else {
        return depWordLower.includes(model.filterString.toLowerCase());
      }
    }

    let filtered = depArray;

    if (this.currentFilterString != '') {
      filtered = depArray.map((v) => v).filter(wordTest);
    }
...
view = {
  ...
  asTable_for: function(model, state) {
    ...
    for (let i = 0; i < a.length; i++) {
      ...
      const dep = a[i][0];
      const reg = a[i][2];
      items += `<tr><td>${id_dep}</td><td><b>${dep}</b></td> <td>${reg}</td></tr>`
    }
  ...
  asTags: function(model, state) {
    return `
          ${state.departmentsFilt.map((v) => `<span><b>${v[0]}</b></span>`).join('')}
      `;
  },
